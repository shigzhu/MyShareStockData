...
class EastMoneyAdapter:
    ...
    def _build_stock(self, row: dict, theme_id: str, row_index: int) -> dict:
        ...
        return_5d = min(17.5, max(-8, change_pct * 0.75 + max(0, 6 - row_index) * 0.8))
        return_10d = min(27.5, max(return_5d, change_pct * 1.05 + max(0, 6 - row_index) * 1.1))
        return_20d = min(42, max(return_10d, change_pct * 1.45 + max(0, 6 - row_index) * 1.2))
        distance_from_ma5 = max(0, min(8.8, return_5d * 0.38))
        distance_from_ma10 = max(0, min(14.5, return_10d * 0.32))

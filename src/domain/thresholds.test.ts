...
describe("defaultThresholds", () => {
  it("uses strict auction confirmation defaults", () => {
    expect(defaultThresholds.auction.idealGapPctMin).toBe(1);
  ...
  it("allows up to two limit-up days in the latest five trading days", () => {
    expect(defaultThresholds.stock.maxConsecutiveLimitUps).toBe(2);
  });
});
